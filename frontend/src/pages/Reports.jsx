...
const PALETTE = ["#C27842", "#2563EB", "#16A34A", "#7C3AED", "#F59E0B", "#DC2626", "#0EA5E9", "#A65D24"];
const STAGE_LABEL = {
...
function MonthlyReport({ rows }) {
  if (!rows.length) return <Empty label="No production data yet — process some POs to see monthly trends." />;
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        ...
        <Stat label="Total Started" value={totalStarted.toLocaleString("en-IN")} />
        <Stat label="Total Dispatched" value={totalDispatched.toLocaleString("en-IN")} accent="#16A34A" />
      </div>
      <Card className="p-6">
        <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Pairs · Started vs Dispatched · last {rows.length} month(s)</h2>
        <div className="w-full h-80" data-testid="monthly-chart">
          <ResponsiveContainer>
            <LineChart data={chartData} margin={{ top: 5, right: 16, left: 0, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ fontSize: 12, border: "2px solid #0F172A" }} />
              <Legend wrapperStyle={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }} />
              <Line type="monotone" dataKey="started" stroke="#C27842" strokeWidth={2.5} dot={{ r: 4 }} name="Started" />
              <Line type="monotone" dataKey="dispatched" stroke="#16A34A" strokeWidth={2.5} dot={{ r: 4 }} name="Dispatched" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
...
function KarigarReport({ rows }) {
  if (!rows.length) return <Empty label="No karigar output for current month yet." />;
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Stat label="Active Karigars" value={rows.filter(r => r.pairs > 0).length} />
        <Stat label="Total Pairs Produced" value={totalPairs.toLocaleString("en-IN")} />
        <Stat label="Total Wages Earned" value={inr(totalEarnings)} accent="#C27842" />
      </div>
      <Card className="p-6">
        <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Top 10 Karigars by Output (pairs · current month)</h2>
        <div className="w-full h-96" data-testid="karigar-output-chart">
          <ResponsiveContainer>
            <BarChart data={top} layout="vertical" margin={{ top: 5, right: 24, left: 60, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
              <XAxis type="number" tick={{ fontSize: 11 }} />
              <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fontWeight: 700 }} width={100} />
              <Tooltip contentStyle={{ fontSize: 12, border: "2px solid #0F172A" }} formatter={(v, n) => n === "earnings" ? inr(v) : v} />
...
function VarianceReport({ rows }) {
  ...
  const worst = rows.slice(0, 10).map(r => ({ ...r, label: `${r.style_code} · ${r.po_number}` }));
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-6">
        <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Margin % · 10 worst-performing PO line items</h2>
        <div className="w-full h-80" data-testid="variance-chart">
          <ResponsiveContainer>
            <BarChart data={worst} margin={{ top: 5, right: 16, left: 0, bottom: 60 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
              <XAxis dataKey="label" tick={{ fontSize: 10 }} angle={-25} textAnchor="end" interval={0} height={70} />
              <YAxis tick={{ fontSize: 11 }} unit="%" />
              <Tooltip contentStyle={{ fontSize: 12, border: "2px solid #0F172A" }} formatter={(v) => `${num(v, 1)}%`} />
              <Bar dataKey="margin_pct" name="Margin %">
                {worst.map((r, i) => (
                  <Cell key={i} fill={r.margin_pct < 0 ? "#DC2626" : r.margin_pct < 10 ? "#F59E0B" : "#16A34A"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b-2 border-slate-200"><h2 className="text-sm font-bold uppercase tracking-wider">Detailed table</h2></div>
        <table className="w-full text-sm" data-testid="variance-table">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-[10px] uppercase tracking-wider text-slate-600">
              <th className="px-4 py-2 font-bold">PO</th><th className="px-4 py-2 font-bold">Client</th>
              <th className="px-4 py-2 font-bold">Style</th><th className="px-4 py-2 font-bold text-right">Qty</th>
              <th className="px-4 py-2 font-bold text-right">Computed</th><th className="px-4 py-2 font-bold text-right">PO Price</th>
...
function CycleReport({ rows }) {
  if (!rows.length) return <Empty label="No cycle-time data yet — move production jobs across stages to populate." />;
  const chartData = rows.map(r => ({ label: `${r.from_stage} → ${r.to_stage}`, avg: r.avg_hours, samples: r.samples }));
  return (
    <div className="space-y-5">
      <Card className="p-6">
        <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Avg hours between stage transitions</h2>
        <div className="w-full h-80" data-testid="cycle-chart">
          <ResponsiveContainer>
            <BarChart data={chartData} margin={{ top: 5, right: 16, left: 0, bottom: 70 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
              <XAxis dataKey="label" tick={{ fontSize: 10 }} angle={-25} textAnchor="end" interval={0} height={80} />
              <YAxis tick={{ fontSize: 11 }} unit="h" />
              <Tooltip contentStyle={{ fontSize: 12, border: "2px solid #0F172A" }} formatter={(v) => `${num(v, 1)} hrs`} />
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b-2 border-slate-200"><h2 className="text-sm font-bold uppercase tracking-wider">Per-transition stats</h2></div>
        <table className="w-full text-sm" data-testid="cycle-table">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-[10px] uppercase tracking-wider text-slate-600">
              <th className="px-4 py-2 font-bold">From</th><th className="px-4 py-2 font-bold">To</th>
              <th className="px-4 py-2 font-bold text-right">Samples</th><th className="px-4 py-2 font-bold text-right">Avg (hrs)</th>
              <th className="px-4 py-2 font-bold text-right">Min</th><th className="px-4 py-2 font-bold text-right">Max</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
...
function DefectReport({ data }) {
  ...
  const byTypePie = data.by_type.map(r => ({ name: r.type, value: r.defective }));
  const byStageBar = data.by_stage.map(r => ({ stage: STAGE_LABEL[r.stage] || r.stage, rate: r.defect_rate_pct, defective: r.defective_qty }));
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Stat label="Total Incidents" value={data.totals.total_incidents} />
        <Stat label="Total Defective Pairs" value={data.totals.total_defective} accent="#DC2626" />
        <Stat label="Total Cost" value={inr(data.totals.total_cost)} accent="#C27842" />
      </div>
      <div className="grid lg:grid-cols-2 gap-5">
        <Card className="p-6">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Defect rate % by stage</h2>
          <div className="w-full h-72" data-testid="defect-stage-chart">
            <ResponsiveContainer>
              <BarChart data={byStageBar} margin={{ top: 5, right: 16, left: 0, bottom: 40 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                <XAxis dataKey="stage" tick={{ fontSize: 10 }} angle={-20} textAnchor="end" interval={0} height={50} />
                <YAxis tick={{ fontSize: 11 }} unit="%" />
                <Tooltip contentStyle={{ fontSize: 12, border: "2px solid #0F172A" }} formatter={(v) => `${num(v, 2)}%`} />
                <Bar dataKey="rate" name="Defect %">
                  {byStageBar.map((r, i) => (
                    <Cell key={i} fill={r.rate > 5 ? "#DC2626" : r.rate > 2 ? "#F59E0B" : "#16A34A"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <Card className="p-6">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-4">Defective pairs by type</h2>
          <div className="w-full h-72" data-testid="defect-type-chart">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={byTypePie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={(e) => e.name}>
                  {byTypePie.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}
                </Pie>
                <Tooltip contentStyle={{ fontSize: 12, border: "2px solid #0F172A" }} />
